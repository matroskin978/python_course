...
def print_weather(data, elements):
    ...
    if data['cod'] != 200:
        ...
        elements["start_content_frame"].pack(fill="both", expand=True, side="top", anchor='nw')
        # welcome_label.configure(text=data["message"])
        elements["city_label"].configure(text="")
    else:
        elements["start_content_frame"].pack_forget()
        elements["content_frame"].pack(fill="both", expand=True, side="top", anchor='nw')
...
Восход: {sunrise_time}
Закат: {sunset_time}"""
        elements["data_textbox"].place(x=450, y=150)
